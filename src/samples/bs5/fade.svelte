<script lang="ts">
  import Example from './__example.svelte';
  import Events from './fade-events.svelte';
  import Events_src from './fade-events.svelte';
  import Sample from './fade-sample.svelte';
  import Sample_src from './fade-sample.svelte';
  import Uncontrolled from './fade-uncontrolled.svelte';
  import Uncontrolled_src from './fade-uncontrolled.svelte';
</script>

<h1>Fade</h1>
Bootstrap does not have a fade component, but utility CSS and JS is used in
Carousels, ListGroups, Modals, and Navs.
<Example source={Sample_src}>
  <Sample />
</Example>

<Example title="Events" source={Events_src}>
  <p slot="info">
    Use the on:opening, on:open, on:closing and on:close props for
    callbacks when the Fade has finished opening (entering) or closing
    (exiting).
  </p>
  <Events />
</Example>

<Example title="Uncontrolled" source={Uncontrolled_src}>
  <p slot="info">
    For the most basic use-case, an uncontrolled component can provide the
    functionality wanted without the need to manage/control the state of the
    component. Fade does not require an isOpen prop. Instead pass a
    toggler prop. The toggler prop is a string which will run querySelectorAll
    to find dom elements which will trigger toggle.
  </p>
  <Uncontrolled />
</Example>
