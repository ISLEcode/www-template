...
namespace qrcodegen {
    ...
    //! A QR Code symbol, which is a type of two-dimension barcode. Invented by Denso Wave and described in the ISO/IEC 18004
    //! standard. Instances of this class represent an immutable square grid of black and white cells. The class provides static
    //! factory functions to create a QR Code from text or binary data. The class covers the QR Code Model 2 specification,
    // supporting all versions (sizes) from 1 to 40, all 4 error correction levels, and 4 character encoding modes.
    //!
    //! Ways to create a QR Code object:
    //!   - High level: Take the payload data and call QrCode.encodeText() or QrCode.encodeBinary().
    //!   - Mid level:  Custom-make the list of segments and call QrCode.encodeSegments().
    //!   - Low level:  Custom-make the array of data codeword bytes (including segment headers and final padding, excluding
    //!     error correction codewords), supply the appropriate version number, and call the QrCode() constructor.
    //!
    ...
    export class QrCode {
        ...
        //! Returns a QR Code representing the given binary data at the given error correction level. This function always
        //! encodes using the binary segment mode, not any text mode. The maximum number of bytes allowed is 2953. The smallest
        //! possible QR Code version is automatically chosen for the output. The ECC level of the result may be higher than the
        //! ecl argument if it can be done without increasing the version.
        ...
        //! Returns a QR Code representing the given segments with the given encoding parameters. The smallest possible QR Code
        //! version within the given range is automatically chosen for the output. Iff boostEcl is true, then the ECC level of
        //! the result may be higher than the ecl argument if it can be done without increasing the version. The mask number is
        //! either between 0 to 7 (inclusive) to force that mask, or -1 to automatically choose an appropriate mask (which may be
        //! slow). This function allows the user to create a custom sequence of segments that switches between modes (such as
        //! alphanumeric and byte) to encode text in less space. This is a mid-level API; the high-level API is encodeText() and
        //! encodeBinary().
        ...
        public static encodeSegments(segs: Array<QrSegment>, ecl: QrCode.Ecc,
            ...
            || mask < -1 || mask > 7) throw "Invalid value";
        ...
        public constructor(
            ...
            // The index of the mask pattern used in this QR Code, which is between 0 and 7 (inclusive). Even if a QR Code is
            // created with automatic masking requested (mask = -1), the resulting object still has a mask value between 0 and 7.
        ...
        // Returns the color of the module (pixel) at the given coordinates, which is false for white or true for black. The top
        // left corner has the coordinates (x=0, y=0). If the given coordinates are out of bounds, then false (white) is returned.
        ...
        // Draws this QR Code, with the given module scale and border modules, onto the given HTML canvas element. The canvas's
        // width and height is resized to (this.size + border * 2) * scale. The drawn image is be purely black and white, and
        // fully opaque. The scale must be a positive integer and the border must be a non-negative integer.
...
`
        ...
        // Draws two copies of the format bits (with its own error correction code) based on the given mask and this object's
        ...
        // Draws two copies of the version bits (with its own error correction code), based on this object's version field,
        // iff 7 <= version <= 40.
...
namespace qrcodegen.QrSegment {
    ...
    export class Mode {
        ...
        // Constructor and fields
        private constructor(
            public  readonly modeBits: int, // The mode indicator bits, which is a uint4 value (range 0 to 15).
            private readonly numBitsCharCount: [int,int,int]) {} // Number of character count bits for three different version ranges.
